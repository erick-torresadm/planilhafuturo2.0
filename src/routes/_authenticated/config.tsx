...
function ConfigPage() {
  ...
  async function trocarSenha() {
    ...
    try {
      ...
      toast.error(e.message ?? "Erro ao trocar a senha");
    } finally {
    ...
    <div className="page-container max-w-3xl space-y-5 animate-in">
      ...
      <section className="rounded-xl bg-card border border-border p-5 space-y-4">
        ...
        <Field label="Nome">
          <Input
            defaultValue={p.nome ?? ""}
            onBlur={(e) =>
              e.target.value !== (p.nome ?? "") && save.mutate({ nome: e.target.value })
            }
          />
        </Field>
        <Field label="Renda mensal">
          <MoneyInput
            value={Number(p.renda_mensal) || 0}
            onCommit={(v) => save.mutate({ renda_mensal: v })}
            align="left"
            size="md"
          />
        </Field>
        <Field label="Saldo inicial (base do Fluxo Diário)">
          <MoneyInput
            value={Number(p.saldo_inicial) || 0}
            onCommit={(v) => save.mutate({ saldo_inicial: v })}
            align="left"
            size="md"
          />
        </Field>
        <Field label="Meta renda fixa (por mês)">
          <MoneyInput
            value={Number(p.meta_renda_fixa) || 0}
            onCommit={(v) => save.mutate({ meta_renda_fixa: v })}
            align="left"
            size="md"
          />
        </Field>
        <Field label="Meses de reserva de emergência">
          <Input
            type="number"
            defaultValue={p.meses_reserva_emergencia ?? 6}
            onBlur={(e) => save.mutate({ meses_reserva_emergencia: Number(e.target.value) })}
          />
        </Field>
      </section>
      ...
      <section className="rounded-xl bg-card border border-border p-5 space-y-4">
        ...
        <p className="text-xs text-muted-foreground -mt-2">
          Depois de trocar, use a nova senha para entrar na próxima vez.
        </p>
      </section>
      ...
      <section className="rounded-xl bg-card border border-border p-5 space-y-4">
        ...
        <div className="flex items-center justify-between">
          ...
          <Switch
            ...
            onCheckedChange={(v) => setSound({ ...sound, enabled: v })}
          />
        </div>
        <div>
          ...
          <Slider
            ...
            max={100}
            step={5}
            onValueChange={(v) => setSound({ ...sound, volume: v[0] / 100 })}
          />
        </div>
        <div className="flex items-center justify-between">
          <Label className="text-sm">Modo silencioso (dia útil)</Label>
          <Switch
            checked={sound.silentDaytime}
            onCheckedChange={(v) => setSound({ ...sound, silentDaytime: v })}
          />
        </div>
        <div>
          <div className="text-xs text-muted-foreground mb-2 flex items-center gap-1">
            <Sparkles className="h-3 w-3" /> Testar sons
          </div>
          <div className="grid grid-cols-3 sm:grid-cols-4 lg:grid-cols-5 gap-2">
            {(
              ...
              <button
                key={s}
                onClick={() => playSound(s)}
                className="chip rounded-xl bg-card border border-border hover:bg-muted transition-all capitalize justify-center"
              >
                {s}
              </button>
            ))}
...
function EquipeSection() {
  ...
  async function revoke(id: string) {
    const { error } = await supabase.from("convites").update({ status: "revogado" }).eq("id", id);
    if (error) {
      toast.error(error.message);
      return;
    }
  ...
  return (
    <section className="rounded-xl bg-card border border-border p-5 space-y-4">
      ...
      <p className="text-xs text-muted-foreground -mt-2">
        Convide outra pessoa para administrar o seu workspace. O convidado cria a própria conta e
        alterna para o seu workspace no app.
      </p>
      ...
      <div className="space-y-2">
        ...
        {link && (
          <div className="rounded-lg bg-muted p-3 space-y-2">
            <p className="text-xs text-muted-foreground">
              Envie este link para o ADM (válido por 7 dias):
            </p>
            <div className="flex items-center gap-2">
              <code className="flex-1 min-w-0 truncate rounded-md bg-card border border-border px-2 py-1.5 text-[11px]">
                {link}
              </code>
              <Button size="sm" variant="outline" onClick={copyLink} className="shrink-0">
                {copied ? (
                  <Check className="h-3.5 w-3.5 mr-1 text-positive" />
                ) : (
  ...
  const PLANOS = [
    {
      ...
      detalhe: "12x R$ 29,90",
      features: [
        "Sistema hospedado por 12 meses",
        "Comunidade PlanilhaClub",
        "Calls e desafios semanais",
        "Canal fechado de membros",
      ],
      tag: "Mais popular",
    },
    {
            ...
            <div className="relative flex items-start gap-4">
              ...
              <div className="flex-1">
                ...
                <div className="flex items-center gap-3 mt-3 text-xs">
                  <span className="flex items-center gap-1 text-positive/80">
                    <ShieldCheck className="h-3 w-3" /> Acesso completo
                  </span>
                  <span className="flex items-center gap-1 text-positive/80">
                    <Zap className="h-3 w-3" /> Sem limites
                  </span>
                </div>
              </div>
            </div>
            ...
            <div className="flex items-start gap-3">
              ...
              <div className="flex-1">
                <div className="flex items-center gap-2 flex-wrap">
                  ...
                  <span className="chip bg-warning/15 text-warning text-[10px]">
                    Prazo para apoiar
                  </span>
                </div>
          ...
          <div className="rounded-xl bg-gradient-to-br from-muted to-muted/50 border border-border p-5">
            <div className="flex items-start gap-3">
              ...
              <div className="flex-1">
                ...
                <p className="text-xs text-muted-foreground mt-1">
                  Você ficou positivo e o prazo de 7 dias terminou sem pagamento. Seus dados
                  continuam salvos — escolha um plano abaixo para voltar a usar tudo na hora.
                </p>
              </div>
            </div>
          </div>
          ...
          {PLANOS.map((plano) => {
            ...
            return (
              ...
              >
                ...
                <div
                  className={cn("flex items-center gap-2", (plano.tag || isCurrentPlan) && "mt-2")}
                >
...
function ErickPlanilhaSection() {
  ...
  return (
    <section className="rounded-xl bg-card border border-border p-5 space-y-4">
      ...
      <p className="text-xs text-muted-foreground">
        Versão oficial da planilha em Excel (.xlsx) — R$ 70 <strong>pra sempre</strong> seu. Mesma
        planilha que o Erick usa, sem marca d'água, sem assinatura.
      </p>
